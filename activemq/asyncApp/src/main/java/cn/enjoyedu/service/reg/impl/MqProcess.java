...
public class MqProcess implements IUserReg {
    ...
    private void sendMq(JmsTemplate jmsTemplate, String dest, final String message) {
    ...
    private void sendMq(JmsTemplate jmsTemplate, String dest, final String message, final boolean needReply) {
        jmsTemplate.send(dest, new MessageCreator() {
            public Message createMessage(Session session) throws JMSException {
                Message msg = session.createTextMessage(message);
                if (needReply) {
